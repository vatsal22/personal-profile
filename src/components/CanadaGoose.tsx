  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      setPosition((prev) => {
        ...
        if (newPos.x > window.innerWidth - gooseSize.width || newPos.x < 0) {
          newVelocity.x = -velocity.x;
          didCollide = true;
        }

        // Bottom and top boundaries
        if (newPos.y > window.innerHeight - gooseSize.height || newPos.y < 0) {
        ...
        return {
          ...
          y: Math.max(
            0,
            Math.min(window.innerHeight - gooseSize.height, newPos.y),
          ),
        };
      });
